...
public class CartServiceImpl implements ICartService {
	...
	@Override
	public void deleteItemByCid(Integer cid, Integer uid) {
		Cart result = cartMapper.findByCid(cid);
		// 判断查询结果是否为null
		if (result == null) {
			// 是：抛出CartNotFoundException
		...
		// 调用updateNumByCid(cid, num, modifiedUser, modifiedTime)执行修改数量
		Integer rows = cartMapper.deleteItemByCid(cid, uid);
		if (rows != 1) {
			throw new InsertException("修改商品数量时出现未知错误，请联系系统管理员");
		}
	}
	...
	@Override
	public void delMultiItems(Integer uid, Integer[] cids) {
		Integer rows = cartMapper.deleteItemByCids(cids, uid);
		if (rows != cids.length) {
			throw new InsertException("エラーです");
		}
	}
